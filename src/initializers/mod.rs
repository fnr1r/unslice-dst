...
use crate::{DstCast, DstLayout, cast::dst_cast_nonnull, uninit::UninitMut};

mod slice_from_iter;

pub(crate) trait DstInit<T: ?Sized>: FnOnce(UninitMut<'_, T>) {}
pub(crate) trait HeadInit<T>: FnOnce(&mut MaybeUninit<T>) -> &mut T {}
pub(crate) trait TailInit<T>: FnOnce(&mut [MaybeUninit<T>]) -> &mut [T] {}

impl<T: ?Sized, F: FnOnce(UninitMut<'_, T>)> DstInit<T> for F {}
